...
|---|---|---|---|---|
| A | @ | 216.150.1.1 | Auto/300s | Required: Make srutikndn.in direct to the site |
| CNAME | www | 8d080ffe41d5448c.vercel-dns-016.com. | Auto/300s | Optional: Make www.srutikndn.in also accessible |
...
3. Add records one by one according to the table:
   ...
   - Host Record(Name):
     - Top-level Domain(such as srutikndn.in) use "@"(some suppliers leave empty to represent @).
     - Subdomain(such as srutikndn.in) use prefix "@".
...
- **Subdomain Access**: If the subdomain (such as blog.srutikndn.in) is accessed, usually only the CNAME record in the table needs to be added.
